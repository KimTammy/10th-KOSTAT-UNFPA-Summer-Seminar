...
USA_Mx <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/deathrate_usa.txt", header = T, skip = 2 ) ## fix this
KOR_Mx <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/deathrate_korea.txt", header = T, skip = 2 )                                         ## fix this
...
1),ncol=1)## fix this
    ...
    ## *** Use this methods and ploting methods -> apply my research ##
    ## Age- & Cause- decomposition of life expectancy
    USA_Mx <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/deathrate_usa.txt", header = T, skip = 2 )  #death rate of USA
      Mx1 <- USA_Mx[USA_Mx$Year==2010,3]   #2010, female
      Mx2 <- USA_Mx[USA_Mx$Year==2018,3]   #2018, female
    ...
    cause_count <- read.csv("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/USA_Cause16_counts.csv")                                       
    
## cause 1: Neoplasms
## cause 2: CVD
## cause3: External causes (Ex: suicide, homicide, desperate death, opoid, etc.)
## cause4: other cases
## This table is organized
    
      cause_count1 <- cause_count[cause_count$Year==2010&
                                    cause_count$Sex==2,] #female                                    
      
      cause_count1 <- matrix(c(cause_count1$Count),
                             ncol = length(unique(cause_count1$Cause)), # column: 열, row: 행
                             nrow = length(unique(cause_count1$Age)))  #matrix: 행과 열로 구성된 데이터를 만들겠다는 뜻임. 
    
    row.names(cause_count1) <- 0:110
    
    colnames(cause_count1) <- 1:16
    
    cause_total1 <- rowSums(cause_count1) #각 나이별로 발생한 사건의 총합
    
    cause_prop1 <- apply(cause_count1, 2, function(x){x/cause_total1}) #행렬을 열 기준으로 정규화하여 'cause_prop1'을 생성함.                                        ## fix this
    ...
    cause_prop2 <- apply(cause_count2, 2, function(x){x/cause_total2})                                        ## fix this
      
      ## Now we construct the cause-specific factor.
      
      cause_fac1 <- (cause_prop1*Mx1)/
        ifelse((Mx2-Mx1)==0,1,Mx2-Mx1)*ex_decomp                                       ## fix this
      
      cause_fac2 <- (cause_prop2*Mx2)/
        ifelse((Mx2-Mx1)==0,1,Mx2-Mx1)*ex_decomp                                        ## fix this
...
### Vaupel & Canudas-Romo Decomposition of Life Expectancy    
   ...
   Y2 <- 1955
    ...
    LT1 <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/lifeexpectancy_sweden.txt", header = T, skip = 2 )                                       ## fix this
      
      LT1 <- LT1[LT1$Year==Y1,]
    
    LT2 <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/lifeexpectancy_sweden.txt", header = T, skip = 2 )                                           ## fix this
    ...
    rho <-  -log(LT2$mx/LT1$mx)/tp                                      ## fix this
      
      
      fx <-  (LT1$dx/100000*LT2$dx/100000)^0.5                                      ## fix this
    ...
    covar <- sum((rho-sum(rho*fx))*(ex-edag[1])*fx)                                        ## fix this
    ...
    table <- 
      matrix(round(c(LT1$ex[1], LT2$ex[1], gap,
                     sum(rho*fx), edag[1],
                     main, covar, gap2),3),ncol=1)## fix this
    ...
    ggplot()+
      geom_line(mapping = aes(x=0:110, y=rho,
                              color="Mortality Improvements"))+                                       ## fix this
...
CALDecompFunction<-function(Mx1,Mx2,Y,Name1,Name2){
  ...
  for (x in 1:111){
    if (x <(YM+1)){
      ...
      pxCH<-c(log(px2/px1),rep(0,111-x))                                       ## fix this
...
LT1 <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/lifetable_italy.txt", header = T, skip = 2 )                                        ## fix this
...
LT2 <- read.table("C:/Users/Tammy/Documents/10th-KOSTAT-UNFPA-Seminar/Day01/lifetable_france.txt", header = T, skip = 2 )                                         ## fix this
...
Y <- min(range(LT1$Year)[2],range(LT2$Year)[2])

Y1 <- 1909
# This is the year where we start doing the
# decomposition minus one. 

result <- CALDecompFunction(Mx1,Mx2,2020,"Italy","France")                                      ## fix this
...
for (y in 1:111){
  for (x in 1:y){
    CALlxD[x,(111-y+x)]<-CALlxDecomp[x,y]                                  ## fix this
      CALlxDS[x,(111-y+x)]<-sum(CALlxDecomp[(1:x),y])                                ## fix this
  }
}
...
filled.contour(YEARS, Age, t(CALlxDS),
               levels=levels, col=WildColors,## fix this
  key.axes=customAxis(),ylab="Age-contribution",
  xlab="Year",cex.lab=1.2)
